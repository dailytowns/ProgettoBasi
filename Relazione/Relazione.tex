	...
	\begin{figure}[h]
		\resizebox{16cm}{!}{
			...
			\node[entity] (Galassia) {Galassia};
				\node[attribute] (nomeAlternativo) [above right of=Galassia] {NomeAlter} edge (Galassia); 
				\node[attribute, above=10mm] (redshift) [left of=Galassia] {Redshift} edge (Galassia);
				\node[attribute, right=10mm] (nomeGalassia) [above left of=Galassia] {\key{Nome}} edge (Galassia);
				
			\node[relationship] (caratterizza) [below  of=Galassia] {caratterizza} edge[label={[above right](1,1)}] (Galassia); 
			
			\node[entity] (CaratteristicheGalassia) [below of = caratterizza] {CaratteristicheGalassia}  edge[label={[above right](1,1)}, color=red] (caratterizza);
				\node[attribute, left=15mm] (distanza) [above left of=CaratteristicheGalassia] {Distanza} edge (CaratteristicheGalassia); 
				\node[attribute, left=15mm] (Metallicita) [left of=CaratteristicheGalassia] {Metallicità} edge (CaratteristicheGalassia);
				\node[attribute, left=15mm] (Luminosita) [below left of=CaratteristicheGalassia] {Luminosità} edge (CaratteristicheGalassia);
				\node[attribute, right=15mm] (ClasseSpettrale) [above right of = CaratteristicheGalassia] {ClasseSpettrale} edge (CaratteristicheGalassia);
				
			\node[relationship, right=25mm] (riferisce) [right of=CaratteristicheGalassia] {riferisce} edge (CaratteristicheGalassia);
			
			\node[entity, right=15mm] (RiferimentoBibliografico) [right of = riferisce] {RiferimentoBibliografico}  edge[label={[above right](1,1)}] (riferisce);
				\node[attribute, left=15mm] (riferimentoDistanza) [above right of=RiferimentoBibliografico] {RefDistanza} edge (RiferimentoBibliografico); 
				\node[attribute, right=18mm] (riferimentoMetallicita) [right of=RiferimentoBibliografico] {RefMetallicità} edge (RiferimentoBibliografico);
				\node[attribute, above=10mm] (riferimentoLuminosita) [below of=RiferimentoBibliografico] {RefLuminosità} edge (RiferimentoBibliografico);
				
			\node[relationship] (relazioneErrore) [below of=CaratteristicheGalassia] {erroreMisure} edge (CaratteristicheGalassia); 
			
			\node[entity] (Errore) [below of = relazioneErrore] {Errore}  edge[label={[above right](1,1)}, color=red] (relazioneErrore);
				\node[attribute, left=10mm] (distanzaErrore) [left of=Errore] {ErroreDistanza} edge (Errore); 
				\node[attribute, right=15mm] (metallicitaErrore) [right of=Errore] {ErroreMetallicità} edge (Errore);
				\node[attribute, above=10mm] (riferimentoLuminosita) [below of=Errore] {ErroreLuminosità} edge (Errore);
				
			\node[relationship, right=20mm] (posizioneAngolare) [right of=Galassia] {posizioneAngolare} edge (Galassia); 
			
			\node[entity, right=50mm] (CoordinateAngolari) [right of = posizioneAngolare] {CoordinateAngolari}  edge[label={[below left
				](1,1)}, color=red] (posizioneAngolare);
				\node[attribute, right=20mm] (declinazione) [right of=CoordinateAngolari] {Declinazione} edge (CoordinateAngolari); 
				\node[attribute, right=15mm] (ascensioneRetta) [above right of=CoordinateAngolari] {AscensioneRetta} edge (CoordinateAngolari);
			
			\node[relationship, left=100mm] (relazioneFlusso) [below right of=Galassia] {misuraFlusso}; \draw (relazioneFlusso) |- (Galassia);
			
			\node[entity, below=120mm] (Flusso) [below of = relazioneFlusso] {Flusso}  edge[label={[above right](1,1)}, color=red] (relazioneFlusso);
				\node[attribute, left=10mm] (atomo) [left of=Flusso] {Atomo} edge[color=red] (Flusso);
				\node[entity, below left =20mm and 20mm of Flusso] (FlussoRighe) [below of = Flusso] {FlussoRighe};
				\draw[vecArrow] (FlussoRighe) -| (Flusso);
				\node[entity, below right=20mm and 25mm of Flusso] (FlussoContinuo) [below of = Flusso] {FlussoContinuo};
				\draw[vecArrow] (FlussoContinuo) -| (Flusso);
				\node[attribute] (ref160) [above right of=FlussoContinuo] {Riferimento160um} edge (FlussoContinuo);
				\node[attribute, right=20mm] (aperturaContuinuo) [right of=FlussoContinuo] {Apertura} edge (FlussoContinuo);
				
			\node[entity, left=25mm] (FlussoRigheHP) [below of = FlussoRighe] {FlussoRigheHP};
				\draw[vecArrow] (FlussoRigheHP) -| (FlussoRighe);
				\node[attribute, left=10mm] (aperture) [left of=FlussoRigheHP] {Aperture} edge (FlussoRigheHP);
				\node[entity, right=25mm] (FlussoRigheSpitzer) [below of = FlussoRighe] {FlussoRigheSpitzer};
				\draw[vecArrow] (FlussoRigheSpitzer) -| (FlussoRighe);
				\node[attribute] (IRS) [below of=FlussoRigheSpitzer] {IRSMode} edge (FlussoRigheSpitzer);
				
			\node[relationship, right=80mm] (misuraFlusso) [right of=Flusso] {misuraFlusso} edge (Flusso);
				\node[entity, right=40mm] (Misura) [right of = misuraFlusso] {Misura};
					\node[attribute, right=20mm] (valore) [right of=
					Misura] {Valore} edge (Misura); 
					\node[attribute] (erroreValore) [above right of=Misura] {Errore} edge (Misura);
					\node[attribute] (riferimento) [below right of=Misura] {Riferimento} edge (Misura);
					
				\node[entity, below left=50mm and 10mm] (MisuraFlusso) [below left of = Misura] {MisuraFlusso};
					\draw[color=red] (MisuraFlusso) -| (misuraFlusso);
					\draw[vecArrow] (MisuraFlusso) -| (Misura);
				\node[entity, below right = 50mm and 20mm] (MisuraLuminosita) [below right of = Misura] {MisuraLuminosità};
					\draw[vecArrow] (MisuraLuminosita) -| (Misura);
			\end{tikzpicture}
		}
	\caption{Primo schema ER}
	\end{figure}
	
	\begin{itemize}
		\item Nello schema gli archi in \textcolor{red}{rosso} indicano identificazione. Se collegano un'entità con una relazione definiscono identificazione esterna.
	\end{itemize}
